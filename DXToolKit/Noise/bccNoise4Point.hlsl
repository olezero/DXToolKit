...
float4 bccNoiseClassic(float3 X)
{
    // Rotate around the main diagonal. Not a skew transform.
    float4 result = bccNoiseBase(dot(X, float3(2.0 / 3.0, 2.0 / 3.0, 2.0 / 3.0)) - X);
...
float4 bccNoisePlaneFirst(float3 X)
{
    ...
    float4 result = bccNoiseBase(mul(orthonormalMap, X));
